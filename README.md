...
只给图片也可以。Agent 会先问一句「这张图准备用在哪里、最想达成什么目标」；如果你没有补充，它会根据画面反推可能用途，再给出基于推测的建议。

---

## 效果

- **建议而非裁判**：输出「右上角 logo 偏小、利益点被装饰压住」，而不是生硬打分或下最终结论。
- **最多 3 个重点问题**：强制 Agent 做优先级排序，避免一口气列出十几条让人无从下手。
- **少填信息也能用**：用途、目标、补充要求能给就给；不给时 Agent 会先追问，再按画面推测。
- **可直接改稿**：每条建议都包含位置、为什么重要、怎么改，方便你自己改或转给设计师。
...
- **只给图片时不是完整判断**：可以先追问上下文；如果没有上下文，会按画面推测用途，但无法确认真实 brief、价格、文案、活动规则和品牌规范。
